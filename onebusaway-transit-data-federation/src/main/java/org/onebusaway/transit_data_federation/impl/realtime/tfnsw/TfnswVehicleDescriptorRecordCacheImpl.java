 ...
 */
package org.onebusaway.transit_data_federation.impl.realtime.tfnsw;
 ...
 */
@Component
public class TfnswVehicleDescriptorRecordCacheImpl implements TfnswVehicleDescriptorRecordCache {

    private static int DEFAULT_CACHE_TIMEOUT_SECONDS = 6 * 60;
    private int _cacheTimeoutSeconds = DEFAULT_CACHE_TIMEOUT_SECONDS;
    private Cache<AgencyAndId, TfnswVehicleDescriptorRecord> _vehicleCache;

    @Override
    public void addRecord(TfnswVehicleDescriptorRecord record) {
        if (record == null || record.getVehicleId() == null) {
            // protect the cache against nulls
            return;
        }

        getVehicleCache().put(record.getVehicleId(), record);
    }

    public TfnswVehicleDescriptorRecord getLastRecordForVehicleId(AgencyAndId vehicleId) {
    ...
    private Cache<AgencyAndId, TfnswVehicleDescriptorRecord> getVehicleCache() {
